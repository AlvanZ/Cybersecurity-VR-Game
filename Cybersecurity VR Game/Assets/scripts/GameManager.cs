...
public GameObject spawnParticleSystemPrefab; // Reference to your particle system prefab
    ...
    public void StartNewRound()
{
    ...
    for (int i = 0; i < enemiesToSpawn; i++)
    {
        ...
        // Instantiate the particle system at the enemy spawn location
        if (spawnParticleSystemPrefab != null)
        {
            GameObject spawnEffect = Instantiate(spawnParticleSystemPrefab, spawnPoint.position, Quaternion.identity);
            Destroy(spawnEffect, 5f); // Adjust the time as needed for the duration of the effect
        }
    }
}
...
private IEnumerator WeaponDrop()
{
    ...
    {
        // Destroy the previous weapon at the start of the new round
        if (prev != null)
        {
            Destroy(prev);
        }
        ...
        GameObject newWeapon = Instantiate(selectedWeaponPrefab, new Vector3(weaponlocation.transform.position.x, weaponlocation.transform.position.y + 2, weaponlocation.transform.position.z), Quaternion.identity);

        // Save the reference to the newly instantiated weapon
        prev = newWeapon;
